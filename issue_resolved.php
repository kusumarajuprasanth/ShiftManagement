					...
					<div>
						<div class="body">			
                               ...
                               ?>
									 ...
									 <input type='submit' name='close_issue' value='Close The Issue'><br>
									 </form>
									 <?php 
									 
									 if(isset($_POST['close_issue'])&& isset($_POST['update']) && (isset($_POST['update_info'])&& $_POST['update_info']!='')){
	$time=date('Y-m-d h:i:s');
	$update=$time.' --> ';
	$update.=$_POST['update_info'].'|'.$_POST['update'];
echo"<script>window.alert('issue was closed successfully ')</script>";

    echo "<script>window.location='current_issues.php'</script>";
	
	// print_r($_POST);
		
	$qry="UPDATE `issues` SET  `updates`='{$update}',`issue_status`='closed' WHERE `id`={$row['id']}";
	$resset=mysqli_query($conn,$qry);
