...
$qry="INSERT INTO `employ`(`lmid`, `name`, `mobile`, `project`,`Team_id`) 
VALUES ('{$_POST['emp_id']}','{$_POST['emp_name']}','{$_POST['emp_contact']}','{$_POST['project']}','{$_POST['Team_id']}')";
...
$sid;
$qry2="SELECT `shifts_applicable`, `shifts_start_date`, `starting_shift_id` FROM `teams` WHERE `team_id`={$_POST['Team_id']}";
...
VALUES ('{$sid}','{$_POST['emp_id']}','{$cur_dt}','{$res2[$day]}','','','{$res2[$dayis]}')";
// echo $qry."<br>";
$res=mysqli_query($conn,$qry);
...
if($res==1)
{
	...
	echo'<script>window.location="run_scheduler.php?days=0"</script>';
		...
		echo"<script>window.alert('Employee CREATED ,but Shifts where not allocated ! please contact DEV ')</script>";
}
 
   echo '<script>window.location="create_employ.php"</script>';

}
else {
	echo"<script>window.alert('Employee was NOT CREATED ! please contact DEV ')</script>";
	echo '<script>window.location="create_employ.php"</script>';
}
