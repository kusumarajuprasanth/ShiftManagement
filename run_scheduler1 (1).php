...
$qry="SELECT max(`shift_date`) FROM `shiftschedule` WHERE 1";
...
$total_emp_last_shift_qry="SELECT `lmid`,`shift_id`,`shift_date` from `shiftschedule` where `id` in (SELECT MAX(`id`) FROM `shiftschedule` where `othres_shift`<>'yes' GROUP BY 
`lmid`)";
